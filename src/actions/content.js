          ...
          if (response.data.filter(isAllow).length === 1) {
            ...
            dispatch(
              setOrderValueSuccess({
                docdep: `${response.data[0].lastname} ${
                  response.data[0].firstname
                } ${response.data[0].secondname}`
              })
            );
            dispatch(
              getListDates({
  ...
  return (dispatch, getState) => {
    ...
    if (type === "type") {
      ...
      if (!order[type] || order[type] !== data[type])
        dispatch(getListSpecialization(data[type], true));
      ...
      if (!order[type] || order[type] !== data[type])
        dispatch(getListDoctors(null, data[type], true));
    } else if (type === "docdep_id") {
      if (order.date) dispatch(cleareOrderSuccess("docdep_id"));
      if (!order[type] || order[type] !== data[type])
        dispatch(getListDates(data[type], true));
    }
