	...
	switch (action.type) {
			...
			return {
				...
				order: {
					type:
						action.data === 'spec_id' ||
						action.data === 'servid' ||
						action.data === 'docdep_id' ||
						action.data === 'date'
							? state.order.type
							: null,
					spec_id:
						action.data === 'servid' || action.data === 'docdep_id' || action.data === 'date'
							? state.order.spec_id
							: null,
				...
				orderValues: {
					spec:
						...
						action.data === 'docdep_id' ||
						action.data === 'date'
							? state.orderValues.spec
							: null,
					serv:
						action.data === 'servid' || action.data === 'docdep_id' || action.data === 'date'
							? state.orderValues.serv
							: null,
		...
		case types.SET_PAYLOAD_LINK:
			return {
				...state,
        payLink: action.data,
        payLinkTimeout: (action.code && action.code === 402) ? true : false
			};
