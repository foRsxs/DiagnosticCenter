...
class AuthorizationScreen extends Component {
	...
	authUser = () => {
		let { t } = this.props;
		let number = this.phoneField ? this.phoneField.getRawValue() : this.state.number;
	...
	renderSmsCode() {
		const { formattedNumber, message, timer } = this.state;
		const { t } = this.props;
			...
			<View style={styles.wrapAuthView}>
				<View style={styles.content}>
					<Text style={styles.smsTitle}>
						{t('authorization:auth_sms1')}
						{'\n'}
						{formattedNumber} {t('authorization:auth_sms2')}
					</Text>
					<TextInput
						style={[ styles.inputSMS, { marginBottom: 30 } ]}
						onChangeText={(code) => this.onChangeSms(code)}
						keyboardType="number-pad"
						maxLength={4}
						returnKeyType="done"
					/>
...
export default withNamespaces([ 'authorization', 'common' ])(
	connect(mapStateToProps, mapDispatchToProps)(AuthorizationScreen)
);
