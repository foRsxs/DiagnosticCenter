...
class AuthMethodScreen extends Component {
	...
	_openScan = () => {
		...
		const optionalConfigObject = {
			...
			sensorDescription: this.state.isTouchId
				? t('authorization:auth_type.touch_id')
				: t('authorization:auth_type.face_id'),
	...
	renderConfirmCodeChoose() {
			...
			<View style={styles.wrapConfirmCode}>
				...
				<Content>
					<ListItem
						...
						onPress={() => this.setState({ methods_auth_local: 'code' })}
					>
						<Left>
						...
						<Right>
							<CheckBox
								...
								onPress={() => this.setState({ methods_auth_local: 'code' })}
								checked={methods_auth_local === 'code'}
								color={ACCENT_BLUE}
							/>
						</Right>
					</ListItem>
						...
						>
							...
							<Right>
								<CheckBox
									style={styles.checkStyle}
									onPress={() => this.setState({ methods_auth_local: 'touch' })}
									checked={methods_auth_local === 'touch'}
									color={ACCENT_BLUE}
								/>
							</Right>
						</ListItem>
						...
						>
							...
							<Right>
								<CheckBox
									style={styles.checkStyle}
									onPress={() => this.setState({ methods_auth_local: 'face' })}
									checked={methods_auth_local === 'face'}
									color={ACCENT_BLUE}
								/>
							</Right>
						</ListItem>
        ...
        <View style={styles.btnWrap}>
          <CustomBtn
            ...
            onClick={() =>
              changeMethodsAuth({
                methods_auth: methods_auth_local,
                confirmed: methods_auth_local === 'code' ? false : true
              })}
          />
        </View>
			</View>
		...
		return (
			<View style={styles.wrapScan}>
				<Text style={styles.title}>
					{isTouchId ? t('authorization:auth_touch_id') : t('authorization:auth_face_id')}
				</Text>
	...
	renderPinCode(type) {
		...
		return (
			<View style={styles.wrapPin}>
				<Text style={styles.title}>
					{!pinCode ? t('authorization:pin_create') : t('authorization:pin_input')}
				</Text>
...
export default withNamespaces([ 'authorization', 'common' ])(
	connect(mapStateToProps, mapDispatchToProps)(AuthMethodScreen)
);
